...
@interface MHDayPicker ()<UIPickerViewDataSource,UIPickerViewDelegate>
    ...
    switch (component) {
        ...
        case 2:
        {
            if (self.pickType == DayPickerTypeSolar) {
                ...
                return [NSString stringWithFormat:@"%ld日%@",row+1,weekStr];
            } else {
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view{
    ...
    if (!pickLab) {
        ...
        pickLab.textColor = HexRGBAlpha(0x999999, 1);
        pickLab.font = FONT(17);
    }
    NSString * titleString = [self pickerView:pickerView titleForRow:row forComponent:component];
    
    if ((component == 0 && row == self.selectedYearIndex) || (component == 1 && row == self.selectedMonthIndex) || (component == 2 && row == self.selectedDayIndex)) {
        pickLab.attributedText = [titleString attributedStr:FONT(20) textColor:HexRGBAlpha(0x333333, 1) lineSpace:0 keming:0 aligent:NSTextAlignmentCenter];
    }else{
